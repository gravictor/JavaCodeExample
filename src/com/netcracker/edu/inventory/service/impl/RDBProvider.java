...
public class RDBProvider {
    ...
    protected final String GET_BATTERY_QUERY = "SELECT d.InventoryNumber, d.Type,d.Model , d.Manufacturer , d.ProductionDate,\n" +
            "b.ChargeVolume FROM Battery b INNER JOIN Device d ON d.id = b.device_id WHERE inventorynumber = ?";
    protected final String GET_ROUTER_QUERY = "SELECT d.InventoryNumber, d.Type,d.Model, d.Manufacturer,d.ProductionDate, r.DataRate FROM Device d\n" +
            " INNER JOIN Router R on d.id = R.Device_id\n" +
            " WHERE inventorynumber = ?";
    protected final String GET_SWITCH_QUERY = "SELECT InventoryNumber,Type,Model,Manufacturer,ProductionDate,DataRate,NumberOfPorts,PortsType,Switch.Device_id,NumberOfPorts " +
            "FROM Switch " +
            "JOIN Device ON (Switch.Device_id = Device.id) " +
            "JOIN Router ON (Router.Device_id = Device.id) " +
            "WHERE InventoryNumber = ?";

    protected final String GET_WIFIROUTER_QUERY = "\n" +
            "SELECT d.InventoryNumber, d.Type, d.Model, d.Manufacturer,d.ProductionDate, r.DataRate, w.TechnologyVersion,w.SecurityProtocol,\n" +
            " w.WirelessConnection, w.WirePortType, w.WireConnection\n" +
            "FROM WifiRouter w INNER JOIN Device d ON d.id = w.device_id\n" +
            "INNER JOIN Router R on d.id = R.Device_id\n" +
            "WHERE inventorynumber = ?";
    ...
    protected final String INSERT_DEVICE_QUERY = "INSERT INTO `Device` (`Class`, `InventoryNumber`, `Type`, `Model`, `Manufacturer`, `ProductionDate`) VALUES (?,?,?,?,?,?)";
    ...
    protected final String INSERT_WIFIROUTER_QUERY = "INSERT INTO `WifiRouter` (`Device_id`, `TechnologyVersion`, `SecurityProtocol`, `WirelessConnection`, `WirePortType`, `WireConnection`) VALUES (?,?,?,?,?,?)";
    ...
    protected final String UPDATE_CPK_QUERY = "UPDATE ConnectionPK SET Trunk_seq = ?, SerialNumber = ? WHERE SerialNumber = ?";
    ...
    public Device getDevice(Connection dbConnection, DevicePrimaryKey dpk) throws Exception {
    ...
    public boolean putDevice(Connection dbConnection, Device device) throws SQLException {
        checkDBConnection(dbConnection);
    ...
    public boolean removeDevice(Connection dbConnection, DevicePrimaryKey dpk) throws SQLException {
        checkDBConnection(dbConnection);
        if (dpk == null) {
            return false;
        }
    ...
    protected void fillNetworkElement(Connection dbConnection, NetworkElement element, ResultSet source) throws Exception {
